...
import { Scale, Plus, KeyRound, Trash2, Power } from "lucide-react";
...
export default function JuridicoContas() {
  ...
  const criar = async () => {
    ...
    setSalvando(true);
    const { error } = await supabase.rpc("juridico_criar_usuario", {
  ...
  const novaSenha = async (c: Conta) => {
    ...
    const { error } = await supabase.rpc("juridico_definir_senha", { _id: c.id, _senha: senha });
    if (error) return toast.error(error.message);
  ...
  const excluir = async (c: Conta) => {
    ...
    const { error } = await supabase.from("juridico_usuarios").delete().eq("id", c.id);
    if (error) return toast.error(error.message);
